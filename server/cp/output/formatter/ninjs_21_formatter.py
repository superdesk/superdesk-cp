...
class NINJS21Formatter(Formatter):
    ...
    def _split_combined_subjects(self, subjects):
        ...
        for subject in subjects:
            if "/" in subject.get("name", "") and subject.get("name") in ["Print / Broadcast"]:
    ...
    def _build_descriptions(self, article):
        ...
        if abstract := article.get("abstract"):
            descriptions.append({"role": "abstract", "value": self.sanitize_text(abstract, remove_p_tags=True)})
        ...
        if description_text := article.get("description_text"):
            if role := type_role_mapping.get(article.get("type","")):
                descriptions.append({"role": role, "value": self.sanitize_text(description_text, remove_p_tags=True)})

        # Add HTML descriptions if present
        if description_html := article.get(f"description_html"):
            descriptions.append({"role": "html", "value": self.sanitize_text(description_html, remove_p_tags=False)})
